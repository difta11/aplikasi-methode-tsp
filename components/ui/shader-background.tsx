...
// ShaderBackground — integrasi komponen shader "Neuro Noise" dari 21st.dev
// Shader Builder. Tanpa dependensi: satu canvas WebGL yang mengisi induknya.
// Diadaptasi dari Paper Shaders (Apache-2.0): https://shaders.paper.design/neuro-noise
//
// Penyesuaian untuk MetaPath:
// 1. Warna diganti ke palet aplikasi versi TERANG & samar. Preset aslinya
//    gelap/oranye; latar gelap akan membuat teks hitam di landing tidak terbaca.
// 2. Menghormati prefers-reduced-motion: waktu dibekukan (timeScale 0), jadi
//    gambarnya tetap tampil penuh warna tetapi diam, dan render loop berhenti.
// 3. Nama berkas dibuat deskriptif (aslinya "my-first").
...
// Resep warna MetaPath (palet "Deep Indigo"). Warna ke-2 palet menentukan kabut
// yang menutupi sebagian besar layar — dibuat sangat tipis supaya latar terasa
// seperti tekstur kertas, bukan hiasan yang bersaing dengan konten. Warna ke-3
// (indigo) menentukan filamen, ke-4 (teal) hanya muncul di inti paling terang.
const UNIFORMS = {
  colors: [
    [1.0, 1.0, 1.0], // #ffffff
    [0.91, 0.914, 0.957], // #e8e9f4
    [0.227, 0.227, 0.71], // #3a3ab5
    [0.043, 0.431, 0.388], // #0b6e63
    [0.043, 0.431, 0.388],
    [0.043, 0.431, 0.388],
    [0.043, 0.431, 0.388],
    [0.043, 0.431, 0.388],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Hormati preferensi "kurangi animasi": waktu dibekukan, jadi gambar tetap
    // tampil tapi diam dan loop render berhenti sendiri.
    ...
    // Dideklarasikan sebagai arrow const, bukan `function` biasa: deklarasi
    // fungsi ter-hoist sehingga TypeScript tidak membawa hasil pengecekan null
    // `canvas`/`gl` ke dalamnya (error "possibly null" di mode strict).
    const requestRender = () => {
